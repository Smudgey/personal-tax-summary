...
case class NpsPerson (
  ...
  val isDeceased: Boolean = diedOn.isDefined
  val name: Name = names.sorted.reverse.find{_.endedOn.isEmpty}.getOrElse{
...
object NpsPerson {
  ...
  case class Name(
    ...
    val firstInitial: Char = firstName.head
    val middleInitial: Option[Char] = middleName.map(_.head)
